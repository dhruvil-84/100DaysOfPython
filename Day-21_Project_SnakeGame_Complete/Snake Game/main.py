...
def snakegame():
    """This is the function of the main snake game. It can be recursive based on play again or not."""
    screen.resetscreen()
    set_screen()
    snake = Snake()
    food = Food()
    ...
    game = True
    while game:
        screen.update() # updating the screen only after my snake moves
        # (i.e., not showing the single body of a snake moving)
        ...
        if snake.snake_head.xcor() > 280 or snake.snake_head.xcor() < -280 or snake.snake_head.ycor() > 260 or snake.snake_head.ycor() < -280:
            score.game_over()
            # if you make a replay option and the return value of game over is true,
            # then call the SnakeGame() function otherwise game = false.
            game = False
